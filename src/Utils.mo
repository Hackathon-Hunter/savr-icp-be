...
module {
    ...
    private let ACCOUNT_DOMAIN_SEPERATOR : Blob = "\0Aaccount-id";
    ...
    private func crc32(data : [Nat8]) : Nat32 {
        let table : [Nat32] = [
        ...
        for (byte in data.vals()) {
            let index = Nat32.fromNat(Nat8.toNat(byte ^ Nat8.fromNat(Nat32.toNat(crc & 0xff))));
            crc := (crc >> 8) ^ table[Nat32.toNat(index)];
        };
        crc ^ 0xffffffff;
    };

    public func principalToAccountBlob(principal : Principal) : Blob {
        let principalBlob = Principal.toBlob(principal);
        let subAccount = Blob.fromArray(Array.tabulate<Nat8>(32, func(i) = 0));
        
        let domainSeparatorBytes = Array.append<Nat8>([0x0A], Blob.toArray(Text.encodeUtf8("account-id")));
        let principalBytes = Blob.toArray(principalBlob);
        let subAccountBytes = Blob.toArray(subAccount);
        
        let toHash = Array.flatten<Nat8>([domainSeparatorBytes, principalBytes, subAccountBytes]);
        let hash = sha224(toHash);
        let checksum = crc32(hash);
        
        let checksumBytes = [
            Nat8.fromNat(Nat32.toNat((checksum >> 24) & 0xff)),
            Nat8.fromNat(Nat32.toNat((checksum >> 16) & 0xff)),
            Nat8.fromNat(Nat32.toNat((checksum >> 8) & 0xff)),
            Nat8.fromNat(Nat32.toNat(checksum & 0xff))
        ];
        
        Blob.fromArray(Array.flatten<Nat8>([checksumBytes, hash]));
    };

    private func sha224(data : [Nat8]) : [Nat8] {
        Array.tabulate<Nat8>(28, func(i) = if (i < data.size()) { data[i] } else { 0 });
    };
    ...
    public func blobToHex(blob : Blob) : Text {
        ...
        for (byte in bytes.vals()) {
            let b = Nat8.toNat(byte);
            let high = b / 16;
            let low = b % 16;
            let highChar = if (high < 10) { Nat.toText(high) } else { 
                switch (high) {
                    case (10) { "a" };
                    case (11) { "b" };
                    case (12) { "c" };
            ...
            let lowChar = if (low < 10) { Nat.toText(low) } else { 
                switch (low) {
                    case (10) { "a" };
                    case (11) { "b" };
                    case (12) { "c" };
    ...
    public func hexToBlob(hex : Text) : Blob {
        ...
        var i = 0;
        while (i < chars.size()) {
            let high = charToNat(chars[i]);
            let low = charToNat(chars[i + 1]);
